...
bool karin::menu::touchAction(const QTouchEvent *event)
{
	QGraphicsWidget *w;
		...
		case QEvent::TouchBegin:
			{
				if(m_touchId == -1)
				{
					...
					if(items.size() > 0)
					{
						...
						{
							m_touchAction = (static_cast<button *>(w));
							if(m_touchAction)
							{
								m_touchId = e->id();
								m_lastPos = e->scenePos();
								m_touchAction->touchEvent(true);
								return true;
							}
						}
					}
		...
		case QEvent::TouchCancel:
			{
				if(m_touchId != -1)
				{
					...
					//m_touchAction->touchEvent(false);
					m_lastPos.setX(-1);
					m_lastPos.setY(-1);
					m_touchAction = 0;
					return true;
...
bool karin::menu::sceneEvent(QEvent *e)
{
	switch (e->type()) {
		case QEvent::TouchBegin: 
			{
				touchAction(static_cast<QTouchEvent *>(e));
				return list_view::sceneEvent(e);
			}
		case QEvent::TouchUpdate:
		case QEvent::TouchEnd:
		case QEvent::TouchCancel:
