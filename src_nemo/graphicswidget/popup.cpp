...
karin::popup::popup(QGraphicsWidget *parent)
	...
	m_appearType(karin::popup::Popup_Appear_In_Scene)
{
...
void karin::popup::setParentWidget(QGraphicsWidget *w)
{
	...
	s = scene();
	disconnect(s, SIGNAL(sceneRectChanged(const QRectF &)), this, SLOT(sceneRectChangedSlot(const QRectF &)));

	m_parentWidget = w;
	setParentItem(m_parentWidget);
	s = scene();
	if(s)
		connect(s, SIGNAL(sceneRectChanged(const QRectF &)), this, SLOT(sceneRectChangedSlot(const QRectF &)));
	reset();
}
...
void karin::popup::setPopupType(karin::popup::Popup_Type_e e)
{
	...
	m_popupType = e;
	m_animState.property_name = m_popupType == karin::popup::Popup_Fade ? "opacity" : "geometry";
	m_animation->setPropertyName(m_animState.property_name.toLatin1());
...
void karin::popup::reset(bool close)
{
	...
	if(m_appearType == karin::popup::Popup_Appear_In_Scene)
	{
		...
		if(s)
		{
			...
			if(m_appearType == karin::popup::Popup_Appear_In_Scene)
			{
				if(m_popupType == karin::popup::Popup_R2L || m_popupType == karin::popup::Popup_L2R)
